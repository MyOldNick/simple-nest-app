...
describe('AuthService', () => {
    ...
    describe('login', () => {
        it('should return access and refresh tokens on successful login', async () => {
            ...
            expect(jwtService.signAsync).toHaveBeenCalledWith({ email: mockUser.email, sub: mockUser.id });
        ...
        it('should throw UnauthorizedException if user validation fails', async () => {
            ...
            await expect(authService.login(mockAuthDto)).rejects.toThrow(UnauthorizedException);
        });

        it('should throw InternalServerErrorException on unexpected error', async () => {
            userService.validateUser.mockRejectedValue(new Error('Unexpected error'));

            await expect(authService.login(mockAuthDto)).rejects.toThrow(InternalServerErrorException);
        });
    });
